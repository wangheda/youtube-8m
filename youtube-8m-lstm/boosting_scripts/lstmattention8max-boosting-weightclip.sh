...
model_name="lstmattention8max_boosting"
...
if [ $model_type == "base_model" ]; then
  ...
  CUDA_VISIBLE_DEVICES="$GPU_NO" python train.py \
  ...
  for i in {1..8}; do
    ...
    if [ ! -d $sub_model_dir ]; then
      ...
      CUDA_VISIBLE_DEVICES="$GPU_NO" python train-with-rebuild.py \
    ...
    if [ ! -f $sample_error_file ]; then 
      CUDA_VISIBLE_DEVICES="$GPU_NO" python inference-sample-error.py \
  ...
  for i in {1..2}; do
    ...
    for part in test ensemble_validate ensemble_train; do
      CUDA_VISIBLE_DEVICES="$GPU_NO" python inference-pre-ensemble.py \
