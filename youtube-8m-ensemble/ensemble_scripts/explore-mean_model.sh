            ...
            if [[ "$d3" < "$d4" ]]; then
              for d5 in $(ls $train_path | sort); do
                if [[ "$d4" < "$d5" ]]; then
                  for d6 in $(ls $train_path | sort); do
                    if [[ "$d5" < "$d6" ]]; then
                      for d7 in $(ls $train_path | sort); do
                        if [[ "$d6" < "$d7" ]]; then
                          train_data_patterns="${train_path}/${d1}/*.tfrecord,${train_path}/${d2}/*.tfrecord,${train_path}/${d3}/*.tfrecord,${train_path}/${d4}/*.tfrecord,${train_path}/${d5}/*.tfrecord,${train_path}/${d6}/*.tfrecord,${train_path}/${d7}/*.tfrecord"
                ...
                if [[ "$d4" < "$d5" ]]; then
                  for d6 in $(ls $train_path | sort); do
                    if [[ "$d5" < "$d6" ]]; then
                      for d7 in $(ls $train_path | sort); do
                        if [[ "$d6" < "$d7" ]]; then
                          for d8 in $(ls $train_path | sort); do
                            if [[ "$d7" < "$d8" ]]; then
                              train_data_patterns="${train_path}/${d1}/*.tfrecord,${train_path}/${d2}/*.tfrecord,${train_path}/${d3}/*.tfrecord,${train_path}/${d4}/*.tfrecord,${train_path}/${d5}/*.tfrecord,${train_path}/${d6}/*.tfrecord,${train_path}/${d7}/*.tfrecord,${train_path}/${d8}/*.tfrecord"
